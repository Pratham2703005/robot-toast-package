...
class ToastItem {
  ...
  constructor(id: number, options: RobotToastOptions, onRemove: (id: number) => void) {
    ...
    // Calculate initial robot side based on position and nearScreen
    const baseSide = resolved.position.includes('left') ? 'left' : 'right';
    this.currentRobotSide = resolved.nearScreen ? baseSide : (baseSide === 'left' ? 'right' : 'left');
  ...
  private buildRobot(): HTMLDivElement {
    ...
    if (variant) {
      ...
      const isAllowed = ALLOWED_FORMATS.some(ext => lowerVariant.endsWith(ext));

      if (!isAllowed) {
        console.warn(
          `[RobotToast] robotVariant "${variant}" has an unsupported format. ` +
          `Allowed: svg, png, jpg, jpeg, gif, webp. Falling back to built-in robot.`
        );
  ...
  private buildMessageBox(): HTMLDivElement {
    ...
    // Drag hint (three horizontal lines)
    if (this.options.draggable) {
      const hint = document.createElement('div');
      hint.className = 'robot-toast-drag-hint';
  ...
  private assembleLayout(): void {
    ...
    // Decide order: rtl flips everything, currentRobotSide controls natural order
  ...
  private playEntrance(): void {
    // Step 1 – wrapper becomes visible (opacity handled by robot/message independently)
    ...
    const onRobotEntered = () => {
      ...
      this.robotEl.style.opacity = '1';            // ← add this line FIRST
      this.robotEl.classList.remove(enterClass);
      this.robotEl.classList.add('robot-idle');


      // Step 3 – message pops in after robot is in position
      ...
      const onMsgEntered = () => {
        ...
        // message-enter leaves it at opacity:1 scale:1 – lock that in
...
private afterTypingComplete(): void {
  if (this.autoCloseDuration > 0 && !this.options.hideProgressBar && this.progressBar) {
    this.progressBar.style.animationDuration = `${this.autoCloseDuration}ms`; // ← FIRST
    void this.progressBar.offsetWidth;                                          // ← flush
    this.progressBar.classList.add('robot-toast-progress-auto');               // ← THEN add
...
// resumeTimer() — just un-pause the CSS animation (no restart)
private resumeTimer(): void {
  ...
  private initDrag(): void {
    ...
    const onPointerUp = (e: PointerEvent) => {
      ...
      // ── Snap robot to nearest screen edge ─────────────────────────────
      const wRect    = this.wrapper.getBoundingClientRect();
      const midX     = wRect.left + wRect.width / 2;
      const snapLeft = midX < window.innerWidth / 2;
      const newSide: 'left' | 'right' = snapLeft ? 'left' : 'right';

      // Determine final resting left position (20px margin from edge)
      const finalLeft = snapLeft ? 20 : window.innerWidth - wRect.width - 20;
      ...
      // If the robot switched sides, re-order visually and play snap anim
      if (newSide !== this.currentRobotSide) {
        this.currentRobotSide = newSide;

        // Use CSS order to visually reorder (no DOM detach = no animation restart)
        const robotOnLeft = newSide === 'left';
        this.robotEl.style.order    = robotOnLeft ? '0' : '1';
        this.messageBox.style.order = robotOnLeft ? '1' : '0';

        // Snap animation
        const snapClass = newSide === 'left' ? 'robot-snap-left' : 'robot-snap-right';
        this.robotEl.classList.remove('robot-idle', 'robot-snap-left', 'robot-snap-right');
      ...
      // After the snap glide, remove the inline transition override so
      // future manager-driven shifts work normally again.
